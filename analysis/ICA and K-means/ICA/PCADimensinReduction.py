...
def PCAReduction(dimensions, inputFile):
    for numOfComponents in dimensions:
        ...
        pca = decomposition.PCA(n_components=numOfComponents)
        pca.fit(X)
        X = pca.transform(X)
        ...
        with open(curLoc + '/NewSamples/PCA/afterPCA_' + str(numOfComponents) + '.csv', 'w', newline='') as matrixFile:
        ...
        with open(curLoc + '/NewSamples/PCA/transformingMatrixPCA_' + str(numOfComponents) + '.csv', 'w',
