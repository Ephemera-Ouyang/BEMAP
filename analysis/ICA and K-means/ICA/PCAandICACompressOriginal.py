    ...
    for method in ['PCA', 'ICA']:
        for numOfComponents in dimensions:
            ...
            transforMatrix = loadMatrix(
                curLoc + "/NewSamples/" + str(method) + "/transformingMatrix" + str(method) + "_" + str(
                    numOfComponents) + ".csv")
            b = np.matrix(transforMatrix)
            output = noteMatrixAfterMean * b
            with open(
                    curLoc + '/NewSamples/NoteAfterPCAandICA/using' + str(method) + '_' + str(numOfComponents) + '.csv',
                    'w',
