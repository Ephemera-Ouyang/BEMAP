...
def decide(dimension):
    ...
    # data_initial_file = open(curLoc + "/NewSamples/NoteAfterPCAandICA/usingPCA_" + str(dimension) + ".csv","r")
    # reader = csv.reader(data_initial_file)
    # nums = []
    # for n in reader:
    #     nums.append(n)
    # data_initial_file.close()
    # data_initial = pd.DataFrame(data=nums[1:], columns=nums[0])
